...
serve(async (req) => {
  ...
  const { data: order, error: orderError } = await adminClient
    .from("orders")
    .select("id, user_id, total, created_at, supplier_id, supplier_name, status")
    .eq("id", orderId)
    .single();
  ...
  if (order.supplier_id !== authData.user.id) {
  ...
  const statusLabel = payload.status ?? order.status;
  ...
  if (buyerEmail) {
    const buyerHtml = `
      <h2>Order confirmed: #${orderShortId}</h2>
      <p><strong>Order ID:</strong> ${order.id}</p>
      <p>Hi ${buyerName},</p>
      <p>Your order was confirmed by <strong>${supplierName}</strong>.</p>
      <p><strong>Status:</strong> ${statusLabel}</p>
      <p><strong>Total:</strong> EGP ${Number(order.total).toLocaleString()}</p>
    `;
    await sendEmail(buyerEmail, `Order #${orderShortId} confirmed by ${supplierName}`, buyerHtml);
  }
