...
const NewsletterSection = () => {
  ...
  const title =
    accountType === "supplier" ? t("Stay Informed on Supplier Demand") : t("Stay Informed");
  const description =
    accountType === "supplier"
      ? t("Receive product-performance prompts, inventory reminders, and operational updates tailored to your supplier workflow.")
      : t("Get new product alerts, supplier updates, and curated B2B wellness insights that match your marketplace activity.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          accountType === "supplier"
            ? t("We will send supplier-focused updates to your inbox.")
            : t("We will keep you posted with relevant supplier and product updates."),
      });
      if (!user) setEmail("");
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : t("Unable to subscribe right now.");
      toast({ title: t("Subscription failed"), description: message, variant: "destructive" });
    }
  };
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleSubmit} className="flex gap-2 max-w-lg">
            ...
            >
              ...
              {newsletterMutation.isPending ? t("Saving...") : t("Subscribe")}
            </Button>
          </form>
        </div>
      </div>
