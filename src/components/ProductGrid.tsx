...
const ProductGrid = ({
  ...
  const filtered = useMemo(() => {
    ...
    if (supplierFilter) {
      result = result.filter((product) => product.supplier_id === supplierFilter);
    }
    if (searchFilter) {
    ...
    return result;
  }, [providedProducts, products, categoryFilter, supplierFilter, searchFilter]);
  ...
  useEffect(() => {
    setPage(0);
  }, [categoryFilter, supplierFilter, searchFilter, providedProducts]);
